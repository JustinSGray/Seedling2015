...
**** Add a word about decomposition algorithms that might be needed to perform these design tasks with high-fi.  CO? ****
**** Add Stanford to the help with the MDAO environment, including timeline.  ****
...
Given the success of our existing methods, and for purposes of this NASA LEARN effort, we consider the capabilities of the SU$^2$ framework to be sufficient as they currently exist, and no further improvements are necessary to be able to deal with aerodynamic performance and sonic-boom prediction.  This shape optimization capability, however, must now work in concert with both the ability to reduce airport jet noise below acceptable levels, and the aerostructural analysis of advanced concepts.  These additional interactions must be accounted for in the development of system-level sensitivities and a majority of the effort in this section will be directed towards the enabling of multidisciplinary capabilities in the SU$^2$ framework.  As discussed in the previous section using the example of an aerostructural optimization approach, the majority of the effort envisioned during Phase I will include the development of all necessary sensitivities and partial derivatives that are required to couple SU$^2$ with the TACS solver and its adjoint.

In order to interact, in a design optimization setting, with the reduced-order models of the acoustics of the jet exhaust, we also envision the need to enhance the capabilities of the SU$^2$ framework so that coupled sensitivities that describe the changes in jet noise resulting from variations in the aerodynamic shape (through shielding and scattering) are properly accounted for.  Using the adjoint capabilities included in SU$^2$, which also provides a linearized solver for the RANS equations, together with coupled sensitivity analysis ideas, we believe it is possible to obtain the required sensitivities for integrated design optimization.
    ...
    \subsubsection{Wing \& Fuselage Analysis (Stanford)}
        \begin{itemize}
